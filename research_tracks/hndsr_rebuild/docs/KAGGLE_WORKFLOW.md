...
python research_tracks/hndsr_rebuild/scripts/kaggle_workflow.py run vR.1
```
...
- `research_tracks/hndsr_rebuild/notebooks/versions/kernel-metadata.json`
- `research_tracks/hndsr_rebuild/kaggle/dataset-metadata.json`
...
```bash
python research_tracks/hndsr_rebuild/scripts/validate_notebook_version.py ^
  --version vR.1 ^
  --notebook research_tracks/hndsr_rebuild/notebooks/versions/vR.1_HNDSR.ipynb ^
  --doc research_tracks/hndsr_rebuild/docs/notebooks/vR.1_HNDSR.md ^
  --review research_tracks/hndsr_rebuild/reports/reviews/vR.1_HNDSR.review.md ^
  --config research_tracks/hndsr_rebuild/configs/phase1_sr3_vr1_kaggle.yaml ^
  --smoke-config research_tracks/hndsr_rebuild/configs/phase1_sr3_vr1_smoke.yaml ^
  --control-config research_tracks/hndsr_rebuild/configs/phase0_bicubic_vr1_kaggle_control.yaml
```
...
```bash
cd "c:/Users/harsh/OneDrive - Indian Institute of Information Technology, Nagpur/IIIT Nagpur/5th Semester/Mini Project"
python research_tracks/hndsr_rebuild/scripts/upload_repo_to_kaggle.py
```
...
Edit `research_tracks/hndsr_rebuild/notebooks/versions/kernel-metadata.json`:
```json
{
...
```bash
python research_tracks/hndsr_rebuild/scripts/kaggle_workflow.py push vR.2
```

### Step 6: Monitor & Pull
```bash
# Check status
python research_tracks/hndsr_rebuild/scripts/kaggle_workflow.py status vR.2

# Pull results when done
python research_tracks/hndsr_rebuild/scripts/kaggle_workflow.py pull vR.2
```
...
3. Push the kernel again: `kaggle kernels push -p "research_tracks/hndsr_rebuild/notebooks/versions"`

### "Dataset not found" Error
Do not invent a new dataset name. Fix `research_tracks/hndsr_rebuild/kaggle/dataset-metadata.json` and `kernel-metadata.json` so both reference `harshv777/hndsr-mini-project-code`.
...
| Command | Action |
|---------|--------|
| `python research_tracks/hndsr_rebuild/scripts/kaggle_workflow.py status vR.1` | Check run status |
| `python research_tracks/hndsr_rebuild/scripts/kaggle_workflow.py pull vR.1` | Download results |
| `python research_tracks/hndsr_rebuild/scripts/kaggle_workflow.py list` | List available versions |
| `python research_tracks/hndsr_rebuild/scripts/upload_repo_to_kaggle.py` | Update repo dataset |
...
2. **Upload** → `python research_tracks/hndsr_rebuild/scripts/upload_repo_to_kaggle.py`
4. **Metadata** → Update `kernel-metadata.json` with new version info
5. **Push** → `python research_tracks/hndsr_rebuild/scripts/kaggle_workflow.py push vR.X`
6. **Wait** → 30-60 minutes for full training
7. **Pull** → `python research_tracks/hndsr_rebuild/scripts/kaggle_workflow.py pull vR.X`
